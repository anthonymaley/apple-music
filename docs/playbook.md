...
- **`play track N of playlist X` drops the playlist context** (macOS 26.x, reproduced live) — Apple Music sets `current playlist` to the library, so the playlist stops continuing and "up next" becomes the alphabetical library. Whole-playlist `play playlist X` preserves context; indexed track-play does not. Workaround: build a temp `__queue__ <name>` tail playlist (chosen track + next ~60) and `play playlist` *that*. No clean AppleScript starts a playlist at track N keeping context.
...
## Current Status
**1.9.0 — unified TUI shell.** Bare `music` launches one navigable app: a single `runShell` loop, `Router` scene stack, `ShellFrame`, global keymap, and a background `PlaybackPoller`/`NowPlayingStore`. Three scenes: **Now Playing** (real album-art hero + Up Next from playback context, two-pane), **Playlists** (the 3-zone browser as a tab, `capturesAllInput` filter), **Speakers** (merged picker + per-speaker volume). End-of-queue continuation card menu (Radio/Playlist/Quiet + manual `n`). The 14 slash commands, conversational skill, and status line are unchanged; the shell is only what bare `music` opens in a TTY.

**Architecture decision (researched + verified):** AppleScript (control of Music.app) + REST (catalog/library/recommendations data) is the only viable stack for a no-paid-account, native-macOS CLI. MusicKit/MediaPlayer/MediaRemote/browser are evaluated-and-rejected. R5 (play-track-keeps-playlist-queue), native radio, and a real Up-Next queue are platform/permission gaps, not bugs.

**Unverified-live (next session must confirm before 1.10.0):** R5 temp-`__queue__` fix, the radio Create-Station path (likely Accessibility-blocked from the binary), and the removal of the persistent now-playing bar.
