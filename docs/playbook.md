...
## Current Status
v1.8.0 — **Playlist browser redesign** (`music playlist`): 3-zone surface (rail · hero · preview). Pure `PlaylistBrowserModel.swift` (zones/badges/duration/enrichment-queue/gradient, 21 tests). Progressive tick-driven metadata enrichment (counts + SMART/RADIO/RECENT badges, visible-first, status line, stable reserved column, no layout shift). Hero card (gradient identity block + `N tracks · Hh Mm` + lime actions). Right-panel 8-track preview on cursor-settle. Client-side `/` filter (position-based over a filtered index list; real `plCursor` preserved). Enter→scrollable track list. Five-role palette (cyan/brightWhite/lime/dim/amber — no longer all-green). Perf: bulk `tracks 1 thru n` fetch (3.77s→0.21s on the big playlist). Earlier in this version line: hardened AppleScript escaping; `PollOutcome` poll model (no skip/blank on transient hiccup); SIGWINCH; `clearBody` render fix; dead-code purge; install-id/version/repo-ref cleanup. 14 slash commands, conversational skill, status line.
