...
struct PlaylistList: ParsableCommand {
    ...
    @Flag(name: .long, help: "Output JSON") var json = false
    func run() throws {
        // Per spec: playlist reads are /v1/me/... endpoints requiring user token.
        // Try REST API first (returns richer data), fall back to AppleScript for degraded mode.
        let auth = AuthManager()
        ...
        // Fallback to AppleScript (no auth needed, but local-only)
        let backend = AppleScriptBackend()
        let result = try syncRun {
...
struct PlaylistTracks: ParsableCommand {
    ...
    @Flag(name: .long, help: "Output JSON") var json = false
    func run() throws {
        // Per spec: playlist reads require user token. Try REST API first, AppleScript fallback.
        let auth = AuthManager()
        if let devToken = try? auth.requireDeveloperToken(), let userToken = auth.userToken() {
            let api = RESTAPIBackend(developerToken: devToken, userToken: userToken, storefront: auth.storefront())
            // First find the playlist ID
...
struct PlaylistCreate: ParsableCommand {
    ...
    @Argument(help: "Playlist name") var name: String
    func run() throws {
        // Per spec: playlist create is a /v1/me/... write. Use REST API with user token.
        let auth = AuthManager()
        let devToken = try auth.requireDeveloperToken()
        let userToken = try auth.requireUserToken()
        let api = RESTAPIBackend(developerToken: devToken, userToken: userToken, storefront: auth.storefront())
...
struct PlaylistDelete: ParsableCommand {
    ...
    @Argument(help: "Playlist name") var name: String
    func run() throws {
        // REST API does not expose library playlist deletion.
        // Use AppleScript as the only viable mechanism.
        let backend = AppleScriptBackend()
        _ = try syncRun {
...
struct PlaylistTemp: ParsableCommand {
    ...
    func run() throws {
        ...
        // Add tracks by searching library — match on title AND artist
        ...
        print("Playing temp playlist with \(items.count / 2) tracks. Will auto-delete on next cleanup.")
    }
}
...
struct PlaylistCreateFrom: ParsableCommand {
    ...
    func run() throws {
        ...
        // Add tracks to playlist via AppleScript — match on title AND artist to avoid wrong-track bugs
        for i in stride(from: 0, to: items.count, by: 2) {
            let title = items[i]
            let artist = items[i + 1]
            _ = try syncRun {
...
struct Similar: ParsableCommand {
    ...
    func run() throws {
        // Per spec: discovery commands require both developer + user token.
        let auth = AuthManager()
        let devToken = try auth.requireDeveloperToken()
        let userToken = try auth.requireUserToken()
        let api = RESTAPIBackend(developerToken: devToken, userToken: userToken, storefront: auth.storefront())
        ...
        // Search for the song to get its catalog ID
        let songs = try syncRun { try await api.searchSongs(query: searchQuery, limit: 1) }
        guard let song = songs.first else {
        ...
        // Use the authenticated /v1/me/recommendations endpoint.
        // The API uses the user's listening history + the seed song to generate results.
        // If the recommendations endpoint returns empty, fall back to /v1/catalog/{storefront}/songs/{id}/related
        // which returns related songs from the catalog graph.
        var similar: [CatalogSong] = []

        // Try: GET /v1/me/recommendations?seed-songs={id}&limit={limit}
        let (recData, recStatus) = try syncRun {
            try await api.get("/v1/me/recommendations?limit=\(limit)")
        }
        if (200...299).contains(recStatus) {
            // Parse recommendation albums/playlists for song data
        ...
        // Fallback if recommendations didn't yield results
        if similar.isEmpty {
...
struct Suggest: ParsableCommand {
    ...
    func run() throws {
        // Per spec: requires both tokens for /v1/me/recommendations
        let auth = AuthManager()
        let devToken = try auth.requireDeveloperToken()
        let userToken = try auth.requireUserToken()
        let api = RESTAPIBackend(developerToken: devToken, userToken: userToken, storefront: auth.storefront())

        // Get seed data: either from a playlist's tracks or the current track
        var seedSongIDs: [String] = []

        if let playlistName = from {
            // Get track IDs from the playlist via REST API
            ...
            if let match = items.first(where: {
                ...
                let trackParsed = try JSONSerialization.jsonObject(with: trackData) as? [String: Any]
                let trackItems = trackParsed?["data"] as? [[String: Any]] ?? []
                // Use catalog IDs if available, or search for them
        ...
        // Use /v1/me/recommendations with seed songs
...
struct NewReleases: ParsableCommand {
    ...
    func run() throws {
        // Per spec: requires both tokens. Use /v1/catalog/{storefront}/charts for real new releases.
        let auth = AuthManager()
        let devToken = try auth.requireDeveloperToken()
        let userToken = try auth.requireUserToken()
        let api = RESTAPIBackend(developerToken: devToken, userToken: userToken, storefront: auth.storefront())
        ...
        // Search for the artist to get their catalog ID
...
if [ -z "$ARGS" ]; then
    ...
    # Device names can be multi-word (e.g. "Sonos arc", "Julie office")
    # so we iterate full names and match as substrings of the args
    SPEAKER=""
    ...
    if [ -n "$SPEAKER" ]; then
        # Remove the multi-word speaker name from args (case-insensitive)
        SP_LOWER=$(echo "$SPEAKER" | tr '[:upper:]' '[:lower:]')
        ARGS_LOWER=$(echo "$ARGS" | tr '[:upper:]' '[:lower:]')
        ARGS=$(echo "$ARGS_LOWER" | sed "s/$SP_LOWER//" | sed 's/^ *//;s/ *$//')
        # Restore original casing from the non-speaker portion
        ARGS=$(echo "$ARGUMENTS" | sed -E "s/ *[0-9]+%//" | tr '[:upper:]' '[:lower:]' | sed "s/$SP_LOWER//" | sed 's/^ *//;s/ *$//')
    fi
...
case "$LOWER_INPUT" in
    ...
    airpods)
        ...
        ;;
    *)
        # Parse chained actions with multi-word speaker names.
        # Strategy: split on action keywords (add/remove/only), treat
        # everything between keywords as the speaker name.
        # e.g. "remove sonos arc add julie office" → "remove sonos arc" + "add julie office"
        ACTIONS=""
        CURRENT_ACTION=""
        ...
        [ -n "$CURRENT_ACTION" ] && ACTIONS="${ACTIONS}${ACTIONS:+|}${CURRENT_ACTION}"

        IFS='|' read -ra ACTION_LIST <<< "$ACTIONS"
        for entry in "${ACTION_LIST[@]}"; do
            CMD=$(echo "$entry" | awk '{print $1}' | tr "[:upper:]" "[:lower:]")
            SPEAKER_NAME=$(echo "$entry" | sed 's/^[^ ]* *//')
            case "$CMD" in
                add)    $MUSIC_CLI speaker add "$SPEAKER_NAME" ;;
                remove) $MUSIC_CLI speaker remove "$SPEAKER_NAME" ;;
                only)   $MUSIC_CLI speaker set "$SPEAKER_NAME" ;;
                *)      $MUSIC_CLI speaker set "$entry" ;;
            esac
        done
        ;;
esac`
